...
export class GeminiClient {
  ...
  async reviewCode(code: string, instruction: string, filePath: string): Promise<string> {
  ...
  private buildEditPrompt(request: CodeEditRequest): string {
...
IMPORTANT: Respond with ONLY the complete modified code wrapped in \`\`\`${this.getFileExtension(request.filePath)} code blocks. Do not include any explanations, comments, or additional text outside the code blocks.`;
  ...
  private buildReviewPrompt(code: string, instruction: string, filePath: string): string {
...
Be specific and actionable in your feedback.`;
  ...
  private parseResponse(response: string, filePath: string): CodeEditResponse {
